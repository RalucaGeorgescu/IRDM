    ...
    if os.path.isfile(pairs_path + '/' + name + '_pair_1'):
        ...
        for group_name, group in grouped_data:
            ...
            if permutations.shape[0] > 1:
                sample_size = permutations.shape[0] if permutations.shape[0] < 100 else 100
                sample = np.random.choice(np.arange(permutations.shape[0]), size=sample_size, replace=False)
