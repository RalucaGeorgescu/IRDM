...
class Logistic_Regression(object):
    ...
    def get_MAP_and_NDCG(self, test_x, test_y):
        test_x_np = numpify(test_x).reshape(-1, n_attributes)
        test_y_np = numpify(test_y).reshape(-1, 1)
        predictions = np.argmax(self.__softmax(self.W, test_x_np), axis=1).reshape(test_y_np.shape)
        
        test_y_pd = pd.DataFrame(test_y, columns=['relevance'])
        predictions_pd = pd.DataFrame(predictions, columns=['relevance'])
        test_y_pd['qid'] = test_x['qid']
        predictions_pd['qid'] = test_x['qid']
        return MAP(test_y_pd, predictions_pd).mean_average_precision(), NDCG(test_y_pd, predictions_pd).mean_ndcg()
